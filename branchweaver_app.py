...
def tab_playback(story: Story):
    ...
    # ------------------------------
    # Live DM Assist (context-aware AI)
    # ------------------------------
    st.markdown("### 🧠 Live DM Assist — On-the-Fly NPC Response Generator")
    st.caption("Give the AI what the **players said**, and it will generate an in-character NPC response based on this scene, the NPC, tags, emotions, GM notes, and story history.")

    # Player input
    player_input = st.text_area(
        "What did the players say?",
        placeholder="e.g., 'We tell Grol this is a misunderstanding and Filge sent us.'",
        key="dmassist_player_input",
        height=80,
    )

    # Generate Button
    if st.button("🎭 Generate NPC Response", key="dmassist_btn"):
        try:
            from openai import OpenAI
            client = OpenAI(api_key=st.secrets.get("openai", {}).get("api_key", None))

            # Build context
            playback_hist = st.session_state.ui.get("playback_history", [])
            hist_titles = [story.nodes[nid].title for nid in playback_hist if nid in story.nodes]

            current_id = st.session_state.ui.get("playback_node_id")
            node = story.nodes.get(current_id)

            context_blob = f"""
You are BranchWeaver DM Assist.
Use the following game state to generate an in-character NPC response.
...
1. NPC Response (full dialogue)
2. Scene Shift (how environment reacts)
3. DM Options (3–4 narrative branches)
4. Potential new branch node titles
"""
            )

            text = response.output_text
            st.markdown("#### 🎭 NPC Response")
            st.write(text)
...
def tab_ai_assistant(story: Story):
    st.subheader("🤖 AI Assistant — Story, Scene & Dialogue Generator")
    st.caption("AI-powered tools for node creation, scene writing, dialogue, and continuity.")

    st.markdown("### 🧠 World Lore Memory")
    if "world_lore" not in st.session_state:
        st.session_state.world_lore = {
            ...
            "locations": {},
            "campaign_flags": {},
            "notes": "Add long-term campaign truths here."
        }
    lore_text = st.text_area(
        "Lore JSON (editable)",
        json.dumps(st.session_state.world_lore, indent=2),
        height=240,
        key="lore_block",
    )
    if st.button("💾 Save Lore"):
        try:
            st.session_state.world_lore = json.loads(lore_text)
            st.success("Lore saved.")
        except:
            st.error("Invalid JSON.")

    st.markdown("---")
    st.markdown("### ✍️ AI: Generate or Modify Nodes")
    prompt = st.text_area("Describe what you want to generate", height=120, key="ai_gen_prompt")
    if st.button("✨ Generate", key="ai_gen_btn"):
        st.info("AI integration will generate text here once your OpenAI key is added.")

    st.markdown("---")
    st.markdown("### 🎭 AI Dialogue Generator")
    dialogue_prompt = st.text_area(
        "What did the players say?",
        height=100,
        key="ai_dialogue_prompt",
    )
    if st.button("Generate NPC Reply", key="ai_dialogue_btn"):
